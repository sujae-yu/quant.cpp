/**
 * TurboQuant -- Fused quantize + cache write ROCm/HIP kernels
 *
 ...
 *
 * Mechanically converted from CUDA (tq_fused_cache.cu).
 */
#ifdef TQ_BUILD_ROCM
 ...
 *
 * Each HIP block processes one token's key vector.
    ...
    int head_dim)
{
    ...
    for (int h = 0; h < num_heads; h++) {
        ...
        __shared__ float s_vals[TQ_BK_ROCM];
    ...
    hipStream_t stream)
{
    ...
    hipLaunchKernelGGL((tq_fused_polar_cache_kernel<float, 16>),
        dim3(num_tokens), dim3(threads), 0, stream,
        d_keys,
...
extern "C" void tq_fused_uniform4b_cache_write_rocm(
    ...
    hipStream_t stream)
{
    ...
    hipLaunchKernelGGL((tq_fused_uniform_cache_kernel<float, 16, 4>),
        dim3(num_tokens), dim3(threads), 0, stream,
        d_values,
        d_cache,
        d_slot_mapping,
...
extern "C" void tq_fused_paged_attention_rocm(
    const float* d_query,
    ...
    hipStream_t stream)
{
    ...
    dim3 grid(tq_rocm_num_blocks(seq_len, threads_per_block), num_heads, 1);

    hipLaunchKernelGGL((tq_fused_paged_attention_kernel<16>),
