/**
 * TurboQuant -- PolarQuant ROCm/HIP kernels
 *
 * Quantizes key vectors using polar coordinates (theta, rho) on AMD GPU.
 * Each HIP block processes one quantization group of TQ_BK elements.
 * Uses shared memory for key loading and warp-level reductions
 * for min/max scale computation.
 *
 * Mechanically converted from CUDA (tq_polar.cu).
 */
#ifdef TQ_BUILD_ROCM
    ...
    int head_dim)
{
    ...
    const int block_elems = TQ_BK_ROCM;
...
extern "C" void tq_polar_quantize_rocm(
    ...
    hipStream_t stream)
{
    int num_blocks_q = (n + TQ_BK_ROCM - 1) / TQ_BK_ROCM;
    int threads = TQ_BK_ROCM / 2; /* one thread per pair */

    hipLaunchKernelGGL(tq_polar_quantize_kernel,
        dim3(num_blocks_q), dim3(threads), 0, stream,
        d_keys,
    ...
    hipStream_t stream)
{
    ...
    hipLaunchKernelGGL(tq_polar_attention_kernel,
