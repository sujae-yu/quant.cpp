/**
 * TurboQuant -- Composite ROCm/HIP kernel (PolarQuant + QJL residual)
 *
 ...
 *
 * Mechanically converted from CUDA (tq_turbo.cu).
 */
#ifdef TQ_BUILD_ROCM
 ...
 *
 * Grid:  (num_blocks, 1, 1)
 * Block: (TQ_BK_ROCM/2, 1, 1)  -- one thread per pair
 *
 * This kernel:
    ...
    int head_dim)
{
    ...
    const int pairs     = TQ_BK_ROCM / 2;

    __shared__ float s_keys[TQ_BK_ROCM];
    __shared__ float s_theta[TQ_BK_ROCM / 2];
    __shared__ float s_radius[TQ_BK_ROCM / 2];
    __shared__ float s_params[4]; /* tmin, tmax, rmin, rmax */
    __shared__ float s_residual[TQ_BK_ROCM];
    ...
    __shared__ uint8_t s_packed[TQ_BK_ROCM / 2];
    if (tid < pairs) {
        float t = s_theta[tid];
    ...
    int dim = (TQ_BK_ROCM < head_dim) ? TQ_BK_ROCM : head_dim;
    ...
    int sketches_per_thread = (TQ_SKETCH_DIM_ROCM + pairs - 1) / pairs;
    for (int si = 0; si < sketches_per_thread; si++) {
        int sketch_idx = tid * sketches_per_thread + si;
        if (sketch_idx >= TQ_SKETCH_DIM_ROCM) break;
    ...
    int head_dim)
{
    ...
    __shared__ float s_residual_recon[TQ_BK_ROCM];
    __syncthreads();
    ...
    if (tid < pairs) {
        ...
        for (int c = 0; c < 2; c++) {
            int d = 2 * tid + c;
            float val = 0.0f;
            for (int sk = 0; sk < TQ_SKETCH_DIM_ROCM; sk++) {
    ...
    hipStream_t stream)
{
    ...
    int threads = TQ_BK_ROCM / 2;

    /* Zero output first (QJL hash uses atomicOr) */
    hipMemsetAsync(d_out, 0, num_blocks * sizeof(tq_turbo_block_d), stream);

    hipLaunchKernelGGL(tq_turbo_quantize_kernel,
    ...
    hipStream_t stream)
{
    ...
    hipLaunchKernelGGL(tq_turbo_attention_kernel,
        dim3(seq_len), dim3(threads), 0, stream,
        d_query,
