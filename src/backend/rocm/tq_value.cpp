/**
 * TurboQuant -- Value quantization ROCm/HIP kernels
 *
 ...
 *
 * Mechanically converted from CUDA (tq_value.cu).
 */
#ifdef TQ_BUILD_ROCM
 ...
 *
 * Grid:  (num_blocks, 1, 1)
 * Block: (TQ_BK_ROCM, 1, 1)  -- one thread per element
 *
 * Each HIP block processes one quantization group (TQ_BK elements).
    ...
    int n)
{
    ...
    const int base      = block_idx * TQ_BK_ROCM;
    ...
    float local_val = (tid < TQ_BK_ROCM && global_idx < n) ? val :  1e30f;
    float gmin = blockReduceMin(local_val);
    local_val = (tid < TQ_BK_ROCM && global_idx < n) ? val : -1e30f;
    ...
    if (tid < TQ_BK_ROCM && global_idx < n) {
        ...
        __shared__ uint8_t s_quant[TQ_BK_ROCM];
        ...
        if (tid % 2 == 0) {
            ...
            uint8_t hi = (tid + 1 < TQ_BK_ROCM) ? s_quant[tid + 1] : 0;
    ...
    int n)
{
    ...
    if (tid < TQ_BK_ROCM && global_idx < n) {
        ...
        if (tid % 4 == 0) {
            ...
            if (tid + 1 < TQ_BK_ROCM) byte |= (s_quant[tid + 1] << 2);
            if (tid + 2 < TQ_BK_ROCM) byte |= (s_quant[tid + 2] << 4);
            if (tid + 3 < TQ_BK_ROCM) byte |= (s_quant[tid + 3] << 6);
    ...
    int head_dim)
{
    ...
    int dim_block = d / TQ_BK_ROCM;
    int dim_offset = d % TQ_BK_ROCM;

    float accum = 0.0f;

    int num_dim_blocks = (head_dim + TQ_BK_ROCM - 1) / TQ_BK_ROCM;
    ...
    hipStream_t stream)
{
    ...
    hipLaunchKernelGGL(tq_value_quantize_4b_kernel,
        dim3(num_blocks), dim3(TQ_BK_ROCM), 0, stream,
        d_values,
...
{
    int num_blocks = (n + TQ_BK_ROCM - 1) / TQ_BK_ROCM;
    hipLaunchKernelGGL(tq_value_quantize_2b_kernel,
        dim3(num_blocks), dim3(TQ_BK_ROCM), 0, stream,
        d_values,
...
extern "C" void tq_value_dequant_matmul_4b_rocm(
...
{
    int threads = 256;
    int grid = tq_rocm_num_blocks(head_dim, threads);

    hipLaunchKernelGGL(tq_value_dequant_matmul_4b_kernel,
