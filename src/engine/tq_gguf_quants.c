...
/* ============================================================
 * IQ2_XS dequantization — 74 bytes per 256 elements (2.3125 bpw)
 *
 * Block layout:
 *   d (fp16, 2 bytes): super-block scale
 *   qs[32] (uint16_t, 64 bytes): packed grid index (low 9 bits) + sign index (high 7 bits)
 *   scales[8] (uint8_t, 8 bytes): 4-bit sub-block scales (2 per byte)
 *
 * Uses iq2xs_grid[512] lookup table (9-bit index, distinct from iq2s_grid).
 * Sign decode: ksigns_iq2xs[7-bit qs >> 9] → kmask_iq2xs[8].
 * ============================================================ */
...
static void dequant_iq2_xs(const void* src, float* dst, int n) {
    const int nb = n / 256;
    const uint8_t* base = (const uint8_t*)src;

    for (int b = 0; b < nb; b++) {
        const uint8_t* blk = base + b * 74; /* 74 bytes per block */
        uint16_t d_raw;
        memcpy(&d_raw, blk, 2);
        const float d = fp16_to_fp32(d_raw);
        const uint16_t* qs = (const uint16_t*)(blk + 2);   /* 32 × uint16 */
        const uint8_t*  scales = blk + 2 + 64;             /* 8 × uint8 */
        ...
        for (int ib32 = 0; ib32 < 8; ib32++) {
            ...
            for (int l = 0; l < 4; l++) {
                ...
                for (int j = 0; j < 8; j++) {
                    dst[b * 256 + ib32 * 32 + l * 8 + j] =
                        dl * (float)grid[j] * ((signs & kmask_iq2xs[j]) ? -1.0f : 1.0f);
                }
            }
        }
    }
